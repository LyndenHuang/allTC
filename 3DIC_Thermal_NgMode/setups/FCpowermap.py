...
class FCPowerView:
    ...
    def translate2TilePwr(self, resolution=5):
        ...
        for i in self.dbDict.keys():
            ...
            area = width*height
            pd = (pwr/area)*1000.0
            area = float(self.__F2Sprecision(area, 6))
            pd = float(self.__F2Sprecision(pd, 3))
            ...
            for orx in range(llxID, urxID+1):
                for ory in range(llyID, uryID+1):
                    ...
                    tileDict[_key]["CELLS"].append([i, ratio, pwr, llx, lly, urx, ury, area, pd])
    ...
    def getTilePowerCells(self, resolution, xid, yid, macroList=[], removeMacros=False, outputListName="localCells.csv"):
        ### xid start from 0, yid start from 0 ###
        ### outputList format:  instName,cellName(cell type),llx,lly,urx,ury,pwr ###
        key = "_".join([str(xid), str(yid)])

        outStr = ["##instName,cellName,llx,lly,urx,ury,pwd"]
        #print(self.tileDict[resolution].keys())
        totalPWR = 0.0
        LLX, LLY, URX, URY = 1000000, 1000000, -1000000, -1000000
        for cell in self.tileDict[resolution][key]["CELLS"]:
            isMacroCell = False
            ### cell = [instName, ratio]
            instName = cell[0]
            cellName = self.dbDict[instName]["cellName"]

            llx = self.__F2Sprecision(self.dbDict[instName]["llx"])
            lly = self.__F2Sprecision(self.dbDict[instName]["lly"])
            urx = self.__F2Sprecision(self.dbDict[instName]["urx"])
            ury = self.__F2Sprecision(self.dbDict[instName]["ury"])
            pwr = self.__F2Sprecision(self.dbDict[instName]["PWR"], precision=8)

            if removeMacros:
                if instName in macroList:
                    isMacroCell = True
                else:
                    outStr.append(",".join([instName,cellName,llx,lly,urx,ury,pwr]))
            else:
                outStr.append(",".join([instName,cellName,llx,lly,urx,ury,pwr]))
        ...
        outCellsPath = os.path.join(self.outputFolder, outputListName)
        totalPWR = float(self.__F2Sprecision(totalPWR, precision=6))
        print("<INFO> output local cells to {}, total power(mW): {}".format(outCellsPath, totalPWR))
        with open(outCellsPath, "w") as fid:
            fid.write("\n".join(outStr))
        
        return outCellsPath, [LLX, LLY, URX, URY]

    
    def getTilePowerDist(self, resolution, outputDistName=None):
        tileDict = self.tileDict[resolution]
        _cells = [float(self.__F2Sprecision(tileDict[i]["PWR"], 8)) for i in tileDict.keys()]
        #sortedCells = sorted(_cells, key=lambda x:x[0], reverse=True)
        sortedCells = sorted(_cells, reverse=True)

        if outputDistName is None:
            return sortedCells
        else:
            ### summary output ###
            outStr = ["Power(mW)"]
            for i in sortedCells:
                outStr.append(self.__F2Sprecision(i, 8))
        
            distoutPath = os.path.join(self.outputFolder, outputDistName)
            with open(distoutPath, "w") as fid:
                fid.write("\n".join(outStr))
            
            print("<INFO> output power distribution to {}".format(distoutPath))
            
            return sortedCells
    
    def getTileXStep(self, resolution):
        length = math.ceil((self.URX-self.LLX)/resolution)*resolution
        #print(self.LLX, self.LLY, self.URX, self.URY, length, width)
        Xsize = int(length/resolution)
    ...
    def getTileYStep(self, resolution):
        width = math.ceil((self.URY-self.LLY)/resolution)*resolution
        Ysize = int(width/resolution)
    ...
    def plot(self, ptype=["IPF"],
             highlighted=[], blocked=[], saveImg="pwr.png", isShow=False):
        import matplotlib
        import matplotlib.pyplot as plt
        from matplotlib.patches import Rectangle
        ...
        for i in showDict.keys():
            ...
            if i in highlighted:
                rect = Rectangle((llx, lly), width=width, height=height, facecolor=color_i, edgecolor="yellow", linewidth=1)
            elif i in blocked:
                rect = Rectangle((llx, lly), width=width, height=height, facecolor="k", edgecolor="bisque", linewidth=1)
            else:
                rect = Rectangle((llx, lly), width=width, height=height, facecolor=color_i, edgecolor="white", linewidth=1)
        ...
        im = ax.imshow(pixels, vmin=min_val, vmax=max_val, cmap="coolwarm", extent=[LLX,URX,LLY,URY], origin="lower")
