...
PROJECT = os.path.join(OUTPUT_FOLDER, "cellUPTR_test.aedt")
MODEL_UNIT = "um"
AMBIENT_TEMP="25cel"
NUM_CORES = 1

JSONCONF_PATH = ""
POSTPROCESS_OUTPUT = os.path.join(OUTPUT_FOLDER, "postprocess_test.data")
...
for i in range(num_points_x):
    ...
    #    point_position=[f"{x_pos}um", f"{Y_POS}um", f"{Z_POS}um"], 
    #    monitor_quantity="Temperature",
    #    monitor_name=f"PX_{i+1}"
    #)
...
"""
### Postprocessing ###
temp_Xs = ipk.post.create_field_summary()
temp_Ys = ipk.post.create_field_summary()

for xx in POINT_MONITORS_X:
    temp_Xs.add_calculation(
        entity="Monitor", geometry="Surface", geometry_name=xx, quantity="Temperature"
    )

temp_Xs_data = temp_Xs.get_field_summary_data(pandas_output=True)
temp_Xs_data.head()


surflist = [TOP.top_face_z.id, POWER_LAYER.top_face_z.id]
plot3 = ipk.post.create_fieldplot_surface(
    assignment=surflist, quantity="SurfTemperature"
)
path = plot3.export_image(
    full_path=os.path.join(OUTPUT_FOLDER, "temperature.png"),
    orientation="top",
    show_region=False,
)
